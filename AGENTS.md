...
- `app/engine/game_service.py`
  核心规则层。负责加载最新对局快照，按阶段顺序派发事件，并把结算后的快照交给异步持久化队列；主链路只保留通用默认行为，不写具体 item / passive / map object 效果。
- `app/routes.py`
...
9. 游戏状态以 JSON 快照方式保存。对局内操作先更新进程内最新态并返回，再异步入库；同一进程内的后续请求优先读取最新态，页面刷新后可读取最新态或最后一次已落库快照恢复。
...
- 对局内接口优先保证“操作反馈先到”：移动、使用道具等请求不要被快照入库阻塞；前端应在点击后立刻关闭路径预览、锁定地图和道具按钮，并播放意图动画或加载动画等待后端返回。
- 加载动画应服务于无缝体验：不能出现空白等待或按钮可重复点击；后端返回时不要粗暴中断已开始的关键动画，应在当前阶段自然收束后再进入结算动画和状态刷新。
- 地图缩放、拖拽、路径预览、迷雾和网格渲染都属于高频交互，应避免在 pointer move / slider input 中触发整图重渲染、昂贵布局读取或大量 box-shadow 动画。必要时使用节流、缓存、transform、CSS 变量和轻量占位层。
- 构筑页加载角色立绘时要控制首屏压力：为立绘容器设置稳定尺寸，避免布局跳动；首选角色或当前选中角色可优先加载，其余角色立绘应延迟加载、按需解码或使用低成本占位，避免一次性拉取和解码全部大图导致卡顿。
