...
from app.modules.card_game.content.common.constants import LOCATION_CARD_LIMIT
from app.modules.card_game.content.loader import get_duel_card
from app.modules.card_game.engine.application.build_service import card_by_id as _card_by_id
from app.modules.card_game.engine.ai.player import AiRules, run_ai_turn as _run_ai_turn_impl
from app.modules.card_game.engine.ai.scoring import declaration_selection_ids, selection_card_contribution
...
from app.modules.card_game.engine.setup.snapshot_factory import (
...
def _resolve_genesis_end_of_turn(snapshot: JsonDict, location: JsonDict, side: str) -> None:
    if _location_mark_count(location, side, TAG_GENESIS) <= 0:
        return
    candidates = [
        card
...
def _open_locations(snapshot: JsonDict, side: str) -> list[JsonDict]:
    return [
        location
        for location in snapshot['locations']
        if _is_location_revealed(snapshot, location) and _location_occupied_card_count(location, side) < _location_capacity(location)
    ]
