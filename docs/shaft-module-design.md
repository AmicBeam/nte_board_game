...
- 排轴工具栏通过“循环轴”按钮打开设置弹窗；开启后可按当前队伍角色配置起始能量和起始环合。起始能量受角色自身上限约束，起始环合上限为 100；关闭循环轴时保留配置但不参与结算。
...
- 单次倾陷伤害由四名角色分别贡献。每名角色使用其整轴动作快照的平均倾陷强度、平均穿防、平均减防和平均减抗计算，再乘以倾陷频率计入全队总伤与 DPS；倾陷贡献不计入角色自身伤害、角色自身占比或动作贡献。配装页在角色贡献列表后使用同款行式布局提供倾陷占比与“分析详情”，弹窗展示各角色倾陷贡献、占倾陷比例、单次贡献与平均参数。达芙蒂尔按原表规则额外贡献 1 次自身倾陷伤害。
...
- 九原天赋一按用户 2026-07-19 口径作为创生与创生复制体的独立频率乘区 ×2，与娜娜莉的创生花数量改写、伊洛伊的复制体数量分别相乘；保持单次伤害和各自原持续窗口不变。Nanoka 原始描述为“额外生成1株创生株”，该频率折算属于项目实现口径。
