...
- 排轴工具栏通过“循环轴”按钮打开设置弹窗；开启后可按当前队伍角色配置起始能量、起始环合和该角色已接入的个人资源。起始能量受角色自身上限约束，起始环合上限为 100，个人资源按角色资源上限约束；关闭循环轴时保留配置但不参与结算。
...
- 九原天赋一按用户 2026-07-19 口径作为创生与创生复制体的独立频率乘区 ×2；娜娜莉按用户 2026-07-21 口径只改写原创生花数量，不影响伊洛伊固定20朵的创生复制体。九原频率乘区仍分别作用于二者，并保持单次伤害和各自原持续窗口不变。Nanoka 原始描述为“额外生成1株创生株”，该频率折算属于项目实现口径。
...
| `source_version` | CharField | 该轴最近一次保存时使用的数据版本，例如 `异环云配队 1.0.1`；应用启动或版本发布不得批量改写历史轴 |
...
- 点击广场排轴卡片时先读取公开快照并打开只读时间轴预览，不替换当前编辑工作区。弹窗提供“取消 / 保存到本地”；保存会创建新的私有轴，复制公开快照的备注、轴配置和计算结果，并把标题改为“原标题 - 作者名”（超出标题上限时优先保留完整作者后缀）。
...
- 含非公开角色的公开轴只对白名单账号可见。该限制由服务端统一应用于市场列表、收藏列表和公开轴详情；未登录访客及非白名单账号直接访问轴 ID 时按轴不存在处理。失去白名单权限的账号不能继续发布已有的非公开角色私有轴。
- 未公开角色状态保存在共享数据库的 `ShaftCharacterPublication` 表中，网站与机器人插件使用相同表结构但不互相导入代码。NoneBot `SUPERUSERS` 可通过私聊命令 `公开角色 <角色名>` 将角色全局转为公开；操作同步写库，重启后继续生效。
...
3. 市场接口验证：
   ...
   - 未登录访客和非白名单账号无法从市场、收藏或详情接口读取含非公开角色的公开轴，白名单账号仍可读取。
