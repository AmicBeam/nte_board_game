...
异环风格桌游，采用 Flask 框架实现。

## 游戏特色

整体玩法以魔塔式路线推进为骨架，结合回合制肉鸽的构筑与局内养成。
开局前，玩家需要选择 1 名角色和 x 个异象道具组成初始构筑；角色提供固定面板与被动效果，道具则在对局中以手牌形式参与结算。
进入对局后，玩家会在地图上按回合推进，执行四向移动、使用道具、触发地图事件并与沿途敌人交战。
核心目标是逐步接近异象空间深处的 Boss 区域，利用途中获得的回复、增益和资源完成养成，最终击败关底 Boss。
当前模式分为单人模式和合作房间模式：单人链路已完整打通，合作模式已支持建房、加入、准备和共享回合推进。

## 主要特性

- 使用数据库保存玩家、密码、token、构筑和对局快照。
- 玩家先向QQ群机器人 `Fairy` 发送 `注册账号` 获取密码，再在浏览器中登录。
