...
现行规则、术语、卡牌设计和异能者职责以 `docs/everness-item-chain-card-design.md` 为唯一设计真源。平衡迭代流程、浏览器验收标准和报告索引记录在 `docs/duel-balance-iteration.md`。
...
- 正式体验结论必须通过浏览器完成。
- 浏览器验收需从真实页面进入牌桌，并通过 UI 完成出牌、宣言、选择素材、唤醒异能者、选择目标、结束回合和查看日志。
- 平衡迭代至少保留固定配对对局结果、评分、最低分卡组归因、改动清单和下一轮验证重点。
